...
python KrakenTools-master/extract_kraken_reads.py -k $output_dir/$db/$sample$db\.kraken2 --include-children -s $read_directory/$sample\nonhuman_reads_dedup_5trimmed.1.fastq -s2 $read_directory/$sample\nonhuman_reads_dedup_5trimmed.2.fastq -t $id\  -r $output_dir/$db/$sample$db\.k2report -o $sample\.tid$id\_all.1.fa  -o2 $sample\.tid$id\_all.2.fa  ; done ; done
