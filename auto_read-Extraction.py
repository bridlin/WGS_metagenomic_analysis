...
def main():
    ...
    # Iterate through all files in the kraken_file_path directory
    # and its subdirectories to find Kraken2 reports
    ...
    for root, _, files in os.walk(kraken_file_path):
        ...
        for file in files:
            if file.endswith(".k2report"):  # identification of the Kraken2 reports that have to be read
        ...
        for keys in dict_taxo_per_sample:
            if sample == keys:
                for item in dict_taxo_per_sample[keys]:
                    ...
                    seq_file1 = read_file_path + '/' + striped_sample + '_ADN_contigs_unmatched.fastq'
                    seq_file2 = read_file_path + '/' + striped_sample + '_ADN_contigs_unmatched.fastq'
                    report_file = kraken_file_path + sample + '.k2report'
                    output_file = kraken_file_path + 'extracted_reads/' + sample + '.tid' + str(item) + '.1.fa'
                    output_file2 = kraken_file_path+ 'extracted_reads/' + sample + '.tid' + str(item) + '.2.fa'
                    taxid = [item]
                    print(kraken_file)
                    print(seq_file1)
                    print(seq_file2)
