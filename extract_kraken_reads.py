...
def main():
    ...
    run_extraction(args=args)

def run_extraction(args):
    
    #Start Program
    time = strftime("%m-%d-%Y %H:%M:%S", gmtime())
    logger.info("PROGRAM START TIME: " + time + '\n')
    
    #Check input 
    if (len(args.output_file2) == 0) and (len(args.seq_file2) > 0):
        logger.error("Must specify second output file -o2 for paired input\n")
        raise ValueError("Must specify second output file -o2 for paired input\n")
    ...
    if args.parents or args.children:
        ...
        if args.report_file == "": 
            logger.error(">> ERROR: --report not specified.")
            raise ValueError(">> ERROR: --report not specified.")
        logger.info(">> STEP 0: PARSING REPORT FILE %s\n" % args.report_file)
    ...
    ##############################################################################
    logger.info("\t%i taxonomy IDs to parse\n" % len(save_taxids))
    logger.info(">> STEP 1: PARSING KRAKEN FILE FOR READIDS %s\n" % args.kraken_file)
    #Initialize values
    count_kraken = 0
    read_line = -1
    exclude_taxids = {} 
    ...
    for line in k_file:
        ...
        if (count_kraken % 10000 == 0):
            logger.info('\r\t%0.2f million reads processed' % float(count_kraken/1000000.))
            sys.stdout.flush()
    ...
    #Update user
    k_file.close()
    logger.info('\r\t%0.2f million reads processed\n' % float(count_kraken/1000000.))
    logger.info('\t%i read IDs saved\n' % len(save_readids))
    ##############################################################################
    #Sequence files
    ...
    if len(first) == 0:
        logger.error("ERROR: sequence file's first line is blank\n")
    ...
    if first[0] == ">":
        ...
        logger.error("ERROR: sequence file must be FASTA or FASTQ\n")
        raise ValueError("ERROR: sequence file must be FASTA or FASTQ\n")
    s_file1.close()
    if filetype != 'fastq' and args.fastq_out:
        logger.error('ERROR: for FASTQ output, input file must be FASTQ\n')
        raise ValueError('ERROR: for FASTQ output, input file must be FASTQ\n')
    ####ACTUALLY OPEN FILE
    if(seq_file1[-3:] == '.gz'):
    ...
    logger.info(">> STEP 2: READING SEQUENCE FILES AND WRITING READS\n")
    logger.info('\t0 read IDs found (0 mill reads processed)')
    sys.stdout.flush()
    #Open output file
    if (args.append):
    ...
    for record in SeqIO.parse(s_file1,filetype):
        ...
        if (count_seqs % 1000 == 0):
            logger.info('\r\t%i read IDs found (%0.2f mill reads processed)' % (count_output, float(count_seqs/1000000.)))
            sys.stdout.flush()
        #Check ID 
    ...
    s_file1.close()
    o_file.close()
    logger.info('\r\t%i read IDs found (%0.2f mill reads processed)\n' % (count_output, float(count_seqs/1000000.)))
    sys.stdout.flush()
    if len(seq_file2) > 0:
        count_output = 0
        count_seqs = 0
        logger.info('\t%i read IDs found (%0.2f mill reads processed)' % (count_output, float(count_seqs/1000000.)))
        sys.stdout.flush()
    ...
    #End Program
    logger.info('\t' + str(count_output) + ' reads printed to file\n')
